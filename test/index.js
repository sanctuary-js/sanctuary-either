...
import {Either, Left, Right, either} from '../index.js';
...
test ('util.inspect', () => {
  eq (inspect (Left (['foo', 'bar', 'baz'])),
      'Left (["foo", "bar", "baz"])');
  eq (inspect (Right (['foo', 'bar', 'baz'])),
      'Right (["foo", "bar", "baz"])');
  eq (inspect (Left (Right (Left (Right (-0))))),
      'Left (Right (Left (Right (-0))))');
});

test ('either', () => {
  eq (either (a => 'Left (' + show (a) + ')')
             (b => 'Right (' + show (b) + ')')
             (Left ('abc')),
      'Left ("abc")');
  eq (either (a => 'Left (' + show (a) + ')')
